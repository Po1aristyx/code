...
const handlePay = async (order) => {
  const res = await orderApi.updateOrderStatus({ order_id: order.id, status: 'to_be_shipped' });
  if (res.code === 0) {
...
const handleReceive = async (order) => {
  const res = await orderApi.updateOrderStatus({ order_id: order.id, status: 'completed' });
  if (res.code === 0) {
...
const handleShip = async (order) => {
  const res = await orderApi.updateOrderStatus({ order_id: order.id, status: 'in_transit' });
  if (res.code === 0) {
